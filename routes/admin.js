...
router.get('/markup', verifyToken, isAdmin, async (req, res) => {
  try {
    const result = await db.query('SELECT * FROM markup_settings WHERE type=$1', ['global']);
...
router.put('/payment-requests/:id', verifyToken, isAdmin, async (req, res) => {
  try {
    const { status } = req.body;
    await db.query('UPDATE payment_requests SET status=$1 WHERE id=$2', [status, req.params.id]);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
