    ...
    const result = await db.query(`
      INSERT INTO auctions
        (seller_id, product_name, description, images,
         ...
         end_time, admin_commission_percent)
      VALUES ($1,$2,$3,$4,$5,$5,$6,$7,$8)
      RETURNING *
    `, [req.user.id, product_name, description,
        images || [], starting_price,
        min_bid_increment || 100, end_time, commission]);
...
// END AUCTION + CALCULATE EARNINGS
router.put('/:id/end', verifyToken, isAdmin, async (req, res) => {
  try {
    ...
    const topBid = await db.query(`
      SELECT * FROM auction_bids WHERE auction_id=$1
      ORDER BY bid_amount DESC LIMIT 1
    `, [req.params.id]);

    const winner_id = topBid.rows.length > 0 ? topBid.rows[0].bidder_id : null;
    const final_price = topBid.rows.length > 0 ? topBid.rows[0].bid_amount : a.starting_price;
    const commission = a.admin_commission_percent || 15;
    const admin_earning = final_price * (commission / 100);
    const seller_earning = final_price - admin_earning;

    await db.query(`
      UPDATE auctions SET status='ended', winner_id=$1,
        admin_earning=$2, seller_earning=$3 WHERE id=$4
    `, [winner_id, admin_earning, seller_earning, req.params.id]);

    res.json({ success: true, winner_id, final_price, admin_earning, seller_earning });
  } catch(err) {
    res.status(500).json({ error: 'Failed' });
  }
});
